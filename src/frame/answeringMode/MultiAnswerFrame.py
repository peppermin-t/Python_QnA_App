...
class AnswerFrame(Frame):
    ...
    def __init__(self, parent, user, shuffle_mode=False, bg=bgColor, width=750, height=600, difficulty=None):
        ...
        if difficulty is None:
            ...
            myBank = {}
            for question in user.answerInfo.getQuestionListByDiff(qt.MULTI, difficulty):
                myBank[question] = bank.localBank[qt.MULTI][question]
        ...
        # 切换题目、确认按键，固定
        def confirmChoice():
            isCorrect = user.answer(qt.MULTI, myQuestions[questionIndex], var_r.get())
            if isCorrect:
