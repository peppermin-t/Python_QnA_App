...
class AnswerFrame(Frame):
    ...
    def __init__(self, parent, user, shuffle_mode=False, bg=bgColor, width=750, height=600):
        ...
        def updateWidgets():
            ...
            var_r.set(myBank[myQuestions[questionIndex]])
            note.delete('1.0', END)
        ...
        # 选项，不固定
        var_r = StringVar()
        var_r.set(myBank[myQuestions[0]])
        r1 = Radiobutton(frm, text='T', state=DISABLED, bg=self.bgColor, variable=var_r, value='T')
        r1.grid(row=1, column=0, pady=15)
        r2 = Radiobutton(frm, text='F', state=DISABLED, bg=self.bgColor, variable=var_r, value='F')
